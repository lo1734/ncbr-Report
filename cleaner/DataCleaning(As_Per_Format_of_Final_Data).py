...
# Define fixed category sort order
category_order = [
    "Number of Suicides",
    "% Share",
    "Population (Lakh)",
...
# Sort by Year, State, and Category in custom order
df_long["Category"] = pd.Categorical(df_long["Category"], categories=category_order, ordered=True)
df_long = df_long.sort_values(by=["Year", "State", "Category"])

# Reorder final columns
df_long = df_long[["Year", "State", "Category", "Value", "Unit"]]

# Save to CSV
output_path = os.path.join(cleaned_folder, "NCRB_Statewise_Cleaned(As_Per_Report).csv")
